function J = ObjectiveFCN(u,x,Ts,N,Nu,xxref,u0,p,Q,R,Ru)
...
for ct=1:N
    ...
    xk1 = AUVSys(xk,uk,Ts,1);
   
    % Accumulate state tracking cost from x(k+1) to x(k+N)
    if abs(xk1(1,1)-xxref(ct,1)) < 2
        J = abs(J + (xk1-xxref(ct,:))*Q*(xk1-xxref(ct,:)).'); %cost from final state
    else
        J = abs(J + (xk1-xxref(ct,:))*Q*(xk1-xxref(ct,:)).'); %cost from mean state
    end
